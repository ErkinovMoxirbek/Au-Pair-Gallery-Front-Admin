// src/pages/dashboard/UsersManager.js
...
// Admin/Leitung/Manager foydalanuvchilarini "protected" deb hisoblaymiz
...
export default function UsersManager() {
  ...
  const exportCSV = () => {
    ...
    const csvContent = [header, ...rows]
      .map((e) =>
        e.map((x) => `"${String(x ?? "").replaceAll('"', '""')}"`).join(",")
      )
      .join("\n");
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              ...
              <select
                ...
                className="block w-full sm:w-auto pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-lg bg-white"
              >
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {users.length === 0 ? (
                  ...
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-sm mt-1">
                        Filter/ Suche anpassen oder zurücksetzen.
                      </p>
                      <button
                        onClick={resetFilters}
                ...
                users.map((user) => {
                  ...
                  const statusInfo =
                    STATUS_CONFIG[user.status] || STATUS_CONFIG.NOACTIVE;
                  const isProcessing = processingId === user.id;

                  // Manager sahifasida: admin/sheff/manager uchun actionlar umuman bo'lmasin
                  const canAct = !protectedUser;
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Aktionen: Manager faqat approve/resend. Edit/Delete yo'q */}
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        {protectedUser ? (
                          <span
                            className="inline-flex items-center gap-2 text-gray-300"
                            title="Geschützter Benutzer"
                          >
                          ...
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity focus-within:opacity-100">
                            ...
                            {/* Agar action yo'q bo'lsa ham bo'sh qolmasin */}
                            {canAct &&
                              user.status !== "PENDING" &&
        ...
        <div className="bg-gray-50 px-6 py-4 border-t border-gray-200 flex flex-col sm:flex-row gap-3 sm:justify-between sm:items-center">
          <span className="text-xs text-gray-500 uppercase tracking-wide font-semibold">
            {totalElements} total • Geladen {users.length} • Seite{" "}
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">
                  gültig bis (Lokales Datum/Uhrzeit)
                </label>
                <input
                  type="datetime-local"
